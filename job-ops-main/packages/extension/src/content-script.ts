...
function updatePanel(body: string, badgeText?: string, badgeBg?: string, badgeColor?: string) {
...
function showReadyPanel(jobTitle: string, employer: string, score?: number) {
  ...
  shadow?.getElementById('fill-btn')?.addEventListener('click', () => {
    updatePanel('<div style="text-align:center;color:#666;padding:8px;">Filling application...</div>', 'Filling', '#fff3e0', '#e65100');
...
function doFill() {
  ...
  const profile = {
    ...
    linkedin_url: 'https://linkedin.com/in/johndoe', current_company: 'Acme Corp',
    cover_letter: 'I am excited about this opportunity...',
    salary: '$120,000',
  };

  const atsType = detectAtsByUrl(window.location.href);
  const atsFiller = atsType === 'greenhouse' ? fillGreenhouseForm : fillLeverForm;
  try {
  ...
  console.log('JobOps: label-based filler filled', labelResult.filled, 'fields');

  updatePanel('<div style="text-align:center;font-size:13px;color:#2e7d32;font-weight:500;">\u2713 Fields filled. Please review and submit manually.</div>', 'Review', '#e6f7e6', '#2e7d32');
...
function fillFormByLabels(data: Record<string, string>): { filled: number } {
  const LABEL_MAP: Record<string, string> = {
    ...
    'linkedin profile': 'linkedin_url', company: 'current_company',
    'current company': 'current_company', 'cover letter': 'cover_letter',
    salary: 'salary', 'salary expectations': 'salary',
  };

  let filled = 0;

  const fields = document.querySelectorAll<HTMLElement>('input:not([type="hidden"]):not([type="file"]):not([type="submit"]):not([type="checkbox"]):not([type="radio"]), textarea, select');
    ...
    for (const [pattern, key] of Object.entries(LABEL_MAP)) {
      if (labelLower.includes(pattern) && data[key]) {
        if (field instanceof HTMLSelectElement) {
          ...
          if (option) { field.value = option.value; field.dispatchEvent(new Event('change', { bubbles: true })); filled++; }
        } else {
          const input = field as HTMLInputElement | HTMLTextAreaElement;
          const proto = input.tagName === 'TEXTAREA' ? HTMLTextAreaElement.prototype : HTMLInputElement.prototype;
          const setter = Object.getOwnPropertyDescriptor(proto, 'value')?.set;
          if (setter) { setter.call(input, data[key]); input.dispatchEvent(new Event('input', { bubbles: true })); input.dispatchEvent(new Event('change', { bubbles: true })); filled++; }
        }
        break;
      }
    }
...
function findLabel(el: HTMLElement): string | null {
  ...
  for (let i = 0; i < 5 && parent; i++) {
    const label = parent.querySelector<HTMLElement>(':scope > label, :scope > span, :scope > div');
    if (label && label !== el) {
...
function startConfirmationMonitoring(): void {
  ...
  function done(confirmationId?: string): void {
    if (settled) return;
    settled = true;
    updatePanel('<div style="text-align:center;padding:8px;color:#2e7d32;font-weight:500;">✓ Application submitted successfully!</div>', 'Done', '#e6f7e6', '#2e7d32');
  }
  function fallback(): void {
    if (settled) return;
    settled = true;
    updatePanel('<div style="text-align:center;padding:8px;color:#e65100;font-weight:500;">Did the submission go through?</div><button id="confirm-btn" style="width:100%;padding:8px;background:#2e7d32;color:#fff;border:none;border-radius:8px;cursor:pointer;font-weight:600;margin-top:8px;">Confirm</button>', 'Pending', '#fff3e0', '#e65100');
    const shadow = getShadowRoot();
    shadow?.getElementById('confirm-btn')?.addEventListener('click', () => done());
  }
  function checkUrl(): boolean {
    if (/^\/(confirmation|thank-you|apply\/success)/i.test(window.location.pathname)) { done(); return true; }
    return false;
  }
  function checkDom(): boolean {
    const text = document.body?.innerText ?? '';
    if (/\b(?:Your application has been submitted|Thank you for applying|Application received)\b/i.test(text)) { done(); return true; }
    return false;
  }
  if (checkUrl() || checkDom()) return;
  const observer = new MutationObserver(() => { if (checkUrl() || checkDom()) observer.disconnect(); });
  observer.observe(document.body, { childList: true, subtree: true });
  setTimeout(() => { observer.disconnect(); fallback(); }, 30000);
}
...
async function main() {
  ...
  try {
    ...
    showReadyPanel(prep.job?.title || '', prep.job?.employer || '', prep.job?.suitabilityScore);
  } catch (err) {
    if (panelShown) return;
    panelShown = true;
